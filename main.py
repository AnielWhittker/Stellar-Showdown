...
def main():
  ...
  if st.session_state.button_pressed and st.session_state.sort_result is None:
    sorting_list = list[int(star_range[0]): int(star_range[1])]
    if choice1 == "Distance from Earth" and choice2 == "Merge Sort":
      start = time.time()
      ms.mergeSortDistance(sorting_list, 0, len(sorting_list) - 1)
      end = time.time()
      ...
      start = time.time()
      ms.mergeSortTemperature(sorting_list, 0, len(sorting_list) - 1)
      end = time.time()
      ...
      start = time.time()
      ss.selectionSortDist(sorting_list)
      end = time.time()
      ...
      start = time.time()
      ss.selectionSortTemp(sorting_list)
      end = time.time()
  ...
  if st.sidebar.checkbox("Show stars"):
    ...
    stringStars = ""
    for j in range(len(sorting_list) - 1, len(sorting_list) - 26, -1):
      temperature = str("{:.5f}".format(sorting_list[j].temperature))
      # if the color index was empty, therfore set to 0, set to "N/A"
      if temperature == str(4600 * ((1 / 1.7) + (1 / 0.62))):
        temperature = "N/A"
      stringStars += "Star " + str(sorting_list[j].star_id) + ":" + " Distance from Earth, " + str("{:.5f}".format(sorting_list[j].distance)) + " light-years. Temperature " + temperature + " Kelvin" + "\n"
    st.code(stringStars)
